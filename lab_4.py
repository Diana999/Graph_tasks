#Граф считывается в формате fo. Каждая строчка, что начинается с E обозначает пропускную способность ребра. Чтобы запустить проект нужно 
#Программа вычисляет величину максимального потока между вершинами 
#запустить функцию start(). Пример запуска на последней строчке файла. Использован алгоритм Форда-Фалкерсона. В выходной файл записывается максимальный поток. 
class ReadInput():
    def __init__(self):
